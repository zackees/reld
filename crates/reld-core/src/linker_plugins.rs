...
const SAVE_VAR_NAME: &str = "RELD_SAVE_PLUGIN_OUTPUTS";
...
impl LoadedPlugin {
    fn new(plugin_path: &Path, args: &ElfArgs) -> Result<LoadedPlugin> {
        ...
        if cfg!(target_feature = "crt-static") {
            bail!(
                "Linker plugins cannot be used when Reld was built as a statically linked binary"
            );
        }
        ...
        transfer_vector.push(LdPluginTv {
            ...
            value: reld_plugin_message_callback as *const () as usize,
        });
