...
const SAVE_DIR_ENV: &str = "RELD_SAVE_DIR";
const SAVE_BASE_ENV: &str = "RELD_SAVE_BASE";
const SKIP_LINKING_ENV: &str = "RELD_SAVE_SKIP_LINKING";
...
fn create_symlink(target: &Path, dest_path: &Path) -> Result {
    crate::platforms::fs::create_symlink(target, dest_path).with_context(|| {
        format!(
