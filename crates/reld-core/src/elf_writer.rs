...
use reld_reloc::elf::RISCV_TLS_DTV_OFFSET;
use reld_reloc::elf::RelocationKind;
use reld_reloc::elf::RelocationKindInfo;
use reld_reloc::elf::RelocationSize;
use reld_reloc::elf::SectionFlags;
use reld_reloc::elf::pf;
use reld_reloc::elf::riscvattr::TAG_RISCV_ARCH;
use reld_reloc::elf::riscvattr::TAG_RISCV_PRIV_SPEC;
use reld_reloc::elf::riscvattr::TAG_RISCV_PRIV_SPEC_MINOR;
use reld_reloc::elf::riscvattr::TAG_RISCV_PRIV_SPEC_REVISION;
use reld_reloc::elf::riscvattr::TAG_RISCV_STACK_ALIGN;
use reld_reloc::elf::riscvattr::TAG_RISCV_UNALIGNED_ACCESS;
use reld_reloc::elf::riscvattr::TAG_RISCV_WHOLE_FILE;
use reld_reloc::elf::secnames;
use reld_reloc::elf::secnames::DEBUG_LOC_SECTION_NAME;
use reld_reloc::elf::secnames::DEBUG_RANGES_SECTION_NAME;
use reld_reloc::elf::secnames::DYNSYM_SECTION_NAME_STR;
use reld_reloc::elf::secnames::NOTE_GNU_BUILD_ID_SECTION_NAME_STR;
use reld_reloc::elf::shf;
use reld_reloc::elf::sht;
use reld_reloc::loongarch64::highest_relocation_with_bias;
use reld_reloc::relaxation::RelocationModifier;
use reld_reloc::relaxation::SectionRelaxDeltas;
use reld_reloc::relaxation::opt_input_to_output;
use reld_reloc::utils::slice_from_all_bytes_mut;
...
fn write_program_headers(
    ...
    for segment_layout in &layout.segment_layouts.segments {
        ...
        // Support executable stack (Reld defaults to non-executable stack)
...
>(
    ...
    while let Some(rel) = relocations.next() {
        ...
        modifier = apply_relocation::<A, R, _>(
            object,
            offset_in_section,
            &rel,
            SectionInfo {
        ...
        .with_context(|| {
            format!(
                ...
                display_relocation::<A, R>(object, &rel, layout)
            )
        })?;
        relocation_cache.previous = Some(rel);
    }
...
>(
    ...
    rel: &R,
    section_info: SectionInfo<reld_reloc::elf::SectionFlags>,
    layout: &ElfLayout<'data>,
    out: &mut [u8],
    table_writer: &mut TableWriter,
    trace: &TraceOutput,
    ...
    let mut value = match rel_info.kind {
        ...
        RelocationKind::PairSubtractionULEB128(expected_r_type) => {
            get_pair_subtraction_relocation_value::<A, R>(
                object_layout,
                rel,
                layout,
                resolution,
...
fn write_dynamic_symbol_definitions(table_writer: &mut TableWriter, layout: &ElfLayout) -> Result {
    let chunk_size =
...
fn write_eh_frame_hdr(table_writer: &mut TableWriter, layout: &ElfLayout) -> Result {
...
/// pointer is stored.
fn eh_frame_ptr(layout: &ElfLayout) -> Result<i32> {
...
fn write_section_headers(out: &mut [u8], layout: &ElfLayout) -> Result {
    ...
    while let Some(event) = order.next() {
        ...
        entry.sh_addr.set(
            e,
            if layout.symbol_db.args.should_output_partial_object() {
                0
            } else {
...
fn compute_info_values(layout: &ElfLayout) -> OutputSectionMap<u32> {
