 ...
 * result to reld_handle_plugin_message (defined in linker_plugins.rs).
 */
...
extern void reld_handle_plugin_message(int level, const char *message);

void reld_plugin_message_callback(int level, const char *fmt, ...) {
    va_list args1, args2;
    va_start(args1, fmt);
    va_copy(args2, args1);
    ...
    if (len < 0) {
        ...
        reld_handle_plugin_message(level, fmt);
        return;
    }
    ...
    reld_handle_plugin_message(level, buf);
    free(buf);
}
