...
pub(crate) enum EntryPoint<'a> {
    #[allow(dead_code)] // Phase 3 COFF supports images without an explicit entry symbol.
    None,
    Symbol(&'a [u8]),
    Address(u64),
}
...
pub(crate) trait Args: std::fmt::Debug + Send + Sync + 'static {
    ...
    fn warn_unsupported(&self, opt: &str) -> Result {
        use crate::args::RELD_UNSUPPORTED_ENV;

        let message = format!("{opt} is not yet supported");

        match env::var(RELD_UNSUPPORTED_ENV).unwrap_or_default().as_str() {
            "warn" | "" => self.warning(message),
            "ignore" => {}
            "error" => bail!("{message}"),
            other => bail!("Unsupported value for {RELD_UNSUPPORTED_ENV}={other}"),
        }
        Ok(())
    }
