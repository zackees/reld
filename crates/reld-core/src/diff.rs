//! If the environment variable `RELD_REFERENCE_LINKER` is set, then once we've finished linking,
...
//! enable writing of trace and layout files by the Reld linker, which allow additional information
//! to be added to the diff outputs.
//!
//! For this to work, the linker-diff binary needs to be installed in the same directory as reld.
...
fn run_diff(paths: &BinPaths) -> Result {
    let reld_diff_path = std::env::current_exe()?.with_file_name("linker-diff");

    if !reld_diff_path.exists() {
        bail!("linker-diff binary needs to be in the same directory as reld")
    }

    let mut command = Command::new(reld_diff_path);
    command
        .arg("--reld-defaults")
        .arg("--display-names")
        .arg("reld,ref")
