...
pub const VALIDATE_ENV: &str = "RELD_VALIDATE_OUTPUT";
pub const RELD_UNSUPPORTED_ENV: &str = "RELD_UNSUPPORTED";
pub const WRITE_LAYOUT_ENV: &str = "RELD_WRITE_LAYOUT";
pub const WRITE_TRACE_ENV: &str = "RELD_WRITE_TRACE";
...
/// inconsistency.
pub(crate) const WRITE_VERIFY_ALLOCATIONS_ENV: &str = "RELD_VERIFY_ALLOCATIONS";
...
impl Default for CommonArgs {
    fn default() -> Self {
        Self {
            ...
            print_allocations: env::var("RELD_PRINT_ALLOCATIONS")
                .ok()
...
impl CommonArgs {
    ...
    pub(crate) fn linker_identity(&self) -> String {
        format!("Reld {} (compatible with GNU linkers)", self.version)
    }
...
mod tests {
    ...
    #[test]
    fn test_flavor() {
        let args = Args::new(|| ["ld.reld"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));

        let args = Args::new(|| ["ld64.reld"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));

        let args = Args::new(|| ["reld-link"].into_iter()).unwrap();
        assert!(matches!(args, Args::Coff(_)));

        let args = Args::new(|| ["ld.reld.exe"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));

        let args = Args::new(|| ["ld64.reld.exe"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));

        let args = Args::new(|| ["reld-link.exe"].into_iter()).unwrap();
        assert!(matches!(args, Args::Coff(_)));

        let mut args = Args::new(|| ["reld", "-flavor", "gnu"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));
        args.parse(|| ["reld", "-flavor", "gnu"].into_iter())
            .unwrap();
        assert!(args.common().inputs.is_empty());

        let args = Args::new(|| ["reld", "-flavor", "darwin"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));
        ...
        let args = Args::new(|| ["ld64.reld", "-flavor", "gnu"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));
