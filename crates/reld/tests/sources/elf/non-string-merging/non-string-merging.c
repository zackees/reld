// Defines identical data sections in two different C files and checks that they end up pointing to
// the same memory.
...
void _start(void) {
  ...
  if (s3h != s4h) {
    // Identical custom sections didn't get merged.
    exit_syscall(108);
  }
  if (s3h == s1h) {
    // Different sections got merged when they shouldn't have been.
    exit_syscall(109);
  }
  if (s3h[0] != 'H') {
    exit_syscall(110);
