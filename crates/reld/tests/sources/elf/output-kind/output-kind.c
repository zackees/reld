// The logic of  choosing output kind in LD is so convoluted we sometimes stick to LLD because of
// either the code complexity (and performance) or providing a better user experience. This series
...
// GNU ld creates static PIE only when both `--no-dynamic-linker` and `-pie` are present. There are
// three approaches we can take for this case: emit PIE with incorrect interpreter like GNU ld does
// (at least on x86_64 Linux), emit static PIE like LLD, or emit PIE with correct implicit
