...
//!
//! We depend on Reld's binary output having a corresponding .layout file. This allows us to know
//! all the input sections that Reld put into the binary and where it put them.
//!
...
use reld_reloc::utils::u32_from_slice;
use std::fmt::Display;
...
impl<R: RType> DynamicRelocation<'_, R> {
    ...
    fn normalised(self) -> Self {
        ...
        // Reld currently seems to set undefined dynamic symbols as STT_IFUNC if they resolved to an
                    ...
                    if let Ok(mut bytes) = elf_symbol.name_bytes() {
                        ...
                        if bytes.is_empty() {
                                ...
                                .and_then(|b| A::decode_thunk(b, *merged_value))
                            {
                                ...
                                // directly to the ifunc symbol address (e.g. reld's IPLT
    ...
    fn index_verneed(elf_file: &ElfFile64<'data>) -> Result<Vec<Option<&'data [u8]>>> {
        ...
        while let Some((_verneed, mut aux_iterator)) = verneed_iterator.next()? {
            while let Some(aux) = aux_iterator.next()? {
                let name = aux.name(e, strings)?;
                let index = usize::from(aux.vna_other.get(e));
    ...
    fn index_dynamic_symbols(
        ...
        for sym in elf_file.dynamic_symbols() {
            ...
            let version: Option<&[u8]> = match version_index {
                ...
                Some(version_index) if !version_index.is_special() => self
                    .verdef
                    .get(usize::from(version_index - object::elf::VER_NDX_GLOBAL))
                    .or_else(|| self.verneed.get(usize::from(version_index)))
                    .copied()
                    .flatten(),
                _ => None,
            };
        ...
        dynamic_segment
            ...
            .for_each(|entry| match entry.d_tag.get(e) {
                ...
                object::elf::DT_FLAGS_1
                    if object::elf::DynamicFlags1(entry.val(e)).contains(object::elf::DF_1_PIE) =>
                {
                    self.bin_attributes.output_kind = OutputKind::Executable;
                }
...
fn get_aarch64_tls_offset_bias(file: &object::File) -> u64 {
    use reld_reloc::aarch64::TLS_TCB_SIZE;
...
fn read_segment_elf<'data>(elf_file: &ElfFile64<'data>, address: u64) -> Option<Data<'data>> {
    ...
    for raw_seg in elf_file.elf_program_headers() {
        ...
        if seg_address <= address && address < seg_end {
            ...
            // If the address is beyond the file-backed portion of the segment, it's in a NOBITS
            // region.
            if start >= file_size {
                return Some(Data::Bss);
            }
            let bytes = &file_bytes[file_start + start..file_end];
            return Some(Data::Bytes(bytes));
        }
    }
    None
}
...
fn relocation_num_bytes(info: RelocationKindInfo) -> usize {
    match info.size {
        reld_reloc::elf::RelocationSize::ByteSize(b) => b,
        reld_reloc::elf::RelocationSize::BitMasking(mask) => {
