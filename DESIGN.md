...
200th rebuild of the day*. Dropping the release bar defers LTO, and removes PGO/BOLT interaction,
cross-machine and cross-toolchain reproducible-build guarantees, and the long tail of
compatibility that has historically consumed years — and killed prior efforts outright. It does
**not** relax artifact equivalence for changes intended to preserve linker behavior; see §3.1.
...
The minimum supported Rust version is **1.97.1**. Normal contributor and CI toolchains pin Rust
1.97.1 rather than a floating `stable`; sanitizer and fuzz jobs remain explicit nightly exceptions.
Rust 1.97.1's `cfg_select!` makes every host-platform route exhaustive: an unlisted target fails to
...
### 3.1 Artifact equivalence is a correctness requirement

Reproducible builds and artifact equivalence are different contracts. `reld` does not promise
that separate machines, toolchain versions, environments, or intentionally different linker
policies will produce the same bytes. It **does** require the same effective linker operation to
produce equivalent artifacts.

Because `reld` is a source fork of `wild`, byte identity is the default oracle when a change is
described as implementation-only or performance-only:

- output modes such as mmap, buffered writes, and direct writes must produce byte-identical files;
- parallel and serial implementations of section population, relocation, hashing, and emission
  must produce byte-identical files; and
- a performance-only change must remain byte-identical to its pinned pre-change `reld` baseline
  and, while the behavior remains inherited, the pinned `wild` fork baseline.

Any byte difference in those cases is a correctness failure, not an acceptable benchmark result.
Run each implementation twice first to establish that the test inputs are self-deterministic. A
platform format may contain a genuinely nondeterministic field; if so, the test must identify that
field, explain why deterministic generation is unavailable, normalize only that field, and compare
everything else. A blanket "the format is nondeterministic" exception is not sufficient.

An intentional change to layout, symbol resolution, padding, metadata, build IDs/UUIDs, default
flags, or another output policy may legitimately change bytes. Such a change must declare the
expected delta in its issue or pull request and add all of:

1. a structural format comparison covering headers, sections, symbols, relocations, and load data;
2. a native execution test with exact exit status, stdout, and stderr expectations; and
3. a focused regression test for the policy that justified the byte difference.

Execution is therefore a required backstop, not a substitute for artifact comparison. It cannot
by itself detect corrupt or changed debug information, unexercised relocations, symbol-table
differences, section flags, padding, or build metadata.

The reference must be exact: pin the `wild` commit or pre-change `reld` revision, toolchain,
effective arguments, inputs, and relevant environment. Comparing against whatever `wild` or `lld`
happens to be installed conflates upstream drift with a `reld` change.

### 3.2 Stretch goals — deferred, not rejected
...
- Correctness is a prerequisite to timing. Performance-only linker changes and alternate output
  modes must pass §3.1 artifact identity checks before their numbers are accepted or published
...
The executable oracle used by benchmark replay remains mandatory, but it is only the runtime layer
of the correctness gate. A harness that checks exit status/stdout/stderr without comparing the
artifact does not yet satisfy §3.1. Until the gate is implemented for a platform, equivalence must
be established separately and recorded with the change before a performance result is treated as
accepted.

Platform-specific references and evidence are decentralized so format rules stay explicit:

- [Linux / ELF agent guide](agents/platforms/linux.md)
- [Windows / PE-COFF agent guide](agents/platforms/windows.md)
- [macOS / Mach-O agent guide](agents/platforms/macos.md)
